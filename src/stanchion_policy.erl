...
check_policy(#access_v1{bucket=B,
                        id=Id} = _Access,
             Policy) ->
    
    case check_all_resources(B, Policy) of
        false -> {error, malformedpolicy_resource};
        true ->
            case check_principals(Id, Policy?POLICY.statement) of
                false -> {error, malformedpolicy_principal};
                true -> 
                    case check_actions(Policy?POLICY.statement) of
                        false -> {error, malformedpolicy_action};
                        true -> ok
                    end
            end
    end.
...
check_principals(_, []) -> false;
check_principals(Id, [Stmt|Stmts]) ->
    ?debugVal(Stmt),
    case check_principal(Id, Stmt#statement.principal) of
        true -> true;
        false -> check_principals(Id, Stmts)
    end.    
...
check_principal(_, []) ->
    false;
check_principal(Id, [{canonical_id, Id}|_]) ->
    true;
check_principal(_, [{aws, '*'}|_]) ->
    true;
check_principal(Id, [_|T]) ->
    check_principal(Id, T).
...
reqdata_to_access(RD, Target, ID) ->
    Action = case {wrq:method(RD), Target} of
                 {'PUT', object} ->     's3:PutObject';
                 {'PUT', object_acl} -> 's3:PutObjectAcl';
                 {'PUT', bucket} ->
                     % should be
                     % S3 error: 400 (MalformedPolicy): Policy has invalid action
                     's3:CreateBucket';
...
policy_to_json_term( ?POLICY{ version = V,
      ...
      V =:= <<"2008-10-17">> orelse V =:= undefined ->
    Stmts = lists:map(fun statement_to_pairs/1, Stmts0),
    % hope no unicode included
    Policy0 = [{"Id", ID}, {"Statement",Stmts}],
    Policy = case V of
                 undefined ->  Policy0;
                 <<"2008-10-17">> -> [{"Version", V}|Policy0]
             end,
    list_to_binary(lists:flatten(mochijson2:encode(Policy))).


-spec supported_object_action() -> [object_action()].
supported_object_action() -> ?SUPPORTED_OBJECT_ACTION.

-spec supported_bucket_action() -> [bucket_action()].
...
eval_statement(#access_v1{action=A, req=Req, bucket=B, key=K} = _Access,
...
statement_to_pairs(#statement{sid=Sid, effect=E, principal=P,
                              action=A, not_action=NA, resource=R, condition_block=Cs})->
    AtomE = case E of
                allow -> <<"Allow">>;
                deny  -> <<"Deny">>
            end,
...
-spec binary_to_action(binary()) -> object_action() | bucket_action().
...
parse_principals([{<<"AWS">>,<<"*">>}|TL], Principal) ->
    parse_principals(TL, [{aws, '*'}|Principal]);
parse_principals([{<<"AWS">>,CanonicalIds}|TL], Principal) ->
    CanonicalUsers = lists:map(fun(CanonicalId) -> {canonical_id, binary_to_list(CanonicalId)} end,
                               CanonicalIds),
...
eval_statement_test()->
    ?assert(eval_statement(#access_v1{action='s3:GetObject',
