...
accept_body(RD, Ctx) ->
    Requester = list_to_binary(wrq:get_req_header("X-Rcs-Requester", RD)),
    Bucket = list_to_binary(wrq:path_info(bucket, RD)),
    Body = wrq:req_body(RD),
    %% @TODO Handle json decoding exceptions
    case stanchion_utils:set_bucket_lifecycle(Bucket, Requester, Body) of
        ok ->
            {true, RD, Ctx};
        {error, Reason} ->
...
delete_resource(RD, Ctx) ->
    ...
    case stanchion_utils:delete_bucket_lifefycle(Bucket, RequesterId) of
        ok ->
